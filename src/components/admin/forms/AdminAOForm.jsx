...
const AdminAOForm = ({ ao, onSubmit, isSubmitting, companies = [] }) => {
  const { user } = useAuth();
  const formStorageKey = useMemo(() => `form-admin-ao-${user?.id}-${ao?.id || 'new'}`, [user, ao]);

  const form = useForm({
    resolver: zodResolver(adminAoSchema),
    defaultValues: ao ? {
      ...ao,
  ...
  const handleFormSubmit = (status) => async (data) => {
    const payload = {
      ...data,
