...
export const useAdmin = () => {
    ...
    const handleArticleAction = async(action, articleId, data) => {
        ...
        try {
            ...
            // Récupérer l'ID admin depuis la table admins (sauf pour delete et updateStatus)
            if (action !== 'delete' && action !== 'updateStatus') {
                ...
                if (action === 'create') {
                    const dataToInsert = {...data, admin_id: adminRecord.id };
                    result = await supabase.from('blogs').insert(dataToInsert).select().single();
                } else if (action === 'update') {
                    const dataToUpdate = {...data, admin_id: adminRecord.id };
                    result = await supabase.from('blogs').update(dataToUpdate).eq('id', articleId).select().single();
                }
            } else if (action === 'delete') {
                result = await supabase.from('blogs').delete().eq('id', articleId);
            } else if (action === 'updateStatus') {
                result = await supabase.from('blogs').update({ status: data.status }).eq('id', articleId).select().single();
            } else {
                throw new Error('Action non reconnue');
            }
