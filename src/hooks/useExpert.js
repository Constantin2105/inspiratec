...
export const useExpert = () => {
    ...
    const fetchData = useCallback(
        async(currentProfile, forceRefresh = false) => {
            ...
            try {
                ...
                const promises = [
                    ...
                    .then((res) => ({...res, source: 'aos' })),
                    supabase
                    .from('candidatures_with_details')
                    .select('ao_id, status')
                    .eq('expert_id', expertId)
                    .then((res) => ({...res, source: 'application_statuses' })),
                    supabase
                    .from('notifications')
                    .select('*')
                    .eq('user_id', userId)
                    .order('created_at', { ascending: false })
                    .then((res) => ({...res, source: 'notifications' })),
                    supabase
                    ...
                    .order('created_at', { ascending: false })
                    .limit(3)
                    .then((res) => ({...res, source: 'blogPosts' })),
                    supabase
                    .from('testimonials')
                    .select('*')
                    .eq('author_user_id', userId)
                    .then((res) => ({...res, source: 'testimonials' })),
                ];
