      ...
      return (
        <>
          <Helmet>
            ...
            <div className="space-y-6">
              ...
              <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
                ...
                <TabsContent value="testimonials" className="mt-4">
                  <AdminTestimonials 
                    onSelect={(item) => handleSelect('testimonial', item)}
                    selectedId={activePanel?.type === 'testimonial' ? activePanel.id : null}
                ...
                <TabsContent value="announcements" className="mt-4">
                  <ManageAnnouncements 
                    onSelect={(item) => handleSelect('announcement', item)}
                    selectedId={activePanel?.type === 'announcement' ? activePanel.id : null}
                    refreshKey={refreshKey}
                    onAction={(action, item) => {
                       if(action === 'delete') handleDeleteItem(item);
                    }}
           ...
           <AnimatePresence>
            {activePanel?.type === 'announcement' && (
              <AnnouncementDetailPanel 
                announcement={activePanel} 
                onClose={handleClosePanel} 
                onEdit={handleEditItem}
                onDelete={handleDeleteItem}
              />
            )}
            {activePanel?.type === 'testimonial' && (
              <TestimonialDetailPanel 
                testimonial={activePanel} 
                onClose={handleClosePanel} 
                onEdit={handleEditItem}
                onDelete={handleDeleteItem}
              />
